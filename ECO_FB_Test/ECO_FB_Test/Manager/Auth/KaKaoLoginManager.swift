    ...
    func login(parameter: LoginParam) async throws {
        if UserApi.isKakaoTalkLoginAvailable() { // 카카오톡 앱 실행 가능 여부 확인
            do {
                ...
                if !userExist {
                    let user: User = User(id: id, loginMethod: LoginMethod.kakao.rawValue, isSeller: false, name: "KaKao User", profileImageURL: URL(string: "https://firebasestorage.googleapis.com/v0/b/e-co-4f9aa.appspot.com/o/user%2Fdefault_profile.png?alt=media&token=afe3a2fd-d85b-49c8-8d4d-dcf773e928ef")!, pointCount: 0, cart: [], goodsRecentWatched: [])
